...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
        ...
        target_kl=0.01, logger_kwargs=dict(), save_freq=10, self_agent='agent1',
    ...
    Args:
        ...
        self_agent (str): agent0 is agent 0, agent1 is agent 1
    ...
    if "human" in env_name:
        ...
        env_type = "ant"
    output_dir = os.path.join("backdoor_attack", "multiagent_competition", "fast_failing", "model", env_type)
    ...
    ac = actor_critic(ob_space, ac_space, **ac_kwargs)
    # load ac
    # ac = torch.load('ppo/ppo_s0/pyt_save/model_fast-failing.pt')
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            env.render()
            if self_agent == 'agent0':
                ob0, ob1 = o
                a, v, logp = ac.step(torch.as_tensor(ob0, dtype=torch.float32))
                a1, v1, s1, n1 = torch_policy1.step(ob1, deterministic=True)
                next_o, r, d, _ = env.step([a, a1])
                r, d = r[0], d[0]
                o = ob0

            elif self_agent == 'agent1':
                ob0, ob1 = o
                a0, v0, s0, n0 = torch_policy0.step(ob0, deterministic=True)
                a, v, logp = ac.step(torch.as_tensor(ob1, dtype=torch.float32))
                next_o, r, d, _ = env.step([a0, a])
                r, d = r[1], d[1]
                o = ob1
            else:
                print("You may run in single agent environment")
            ...
            if terminal or epoch_ended:
                ...
                if timeout or epoch_ended:
                    ob0, ob1 = o
                    if self_agent == 'agent0':
                        _, v, _ = ac.step(torch.as_tensor(ob0, dtype=torch.float32))
                    elif self_agent == 'agent1':
                        _, v, _ = ac.step(torch.as_tensor(ob1, dtype=torch.float32))
                    else:
...
if __name__ == '__main__':
    ...
    ppo(lambda: gym.make(args.env), actor_critic=core.MLPActorCritic,
        ...
        logger_kwargs=logger_kwargs, env_name=args.env)